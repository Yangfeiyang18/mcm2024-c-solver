...
def main() -> None:
    ...
    solution = read_csv(RESULTS_DIR / "q1_full_discount_solution_long.csv")
    metrics_path = RESULTS_DIR / "q1_full_discount_metrics.json"
    ...
    minimum_area_by_land_type = {
        ...
        for land_type, value in metrics["management_constraints"]["minimum_area_mu_by_land_type"].items()
    }
    minimum_area_violations = []
    for (year, plot_id, season, crop_id), area in area_by_key.items():
        required = minimum_area_by_land_type[plot_by_id[plot_id]["land_type"]]
        if area > TOLERANCE and area + TOLERANCE < required:
            minimum_area_violations.append((f"{year}/{plot_id}/{season}/{crop_id}", required - area))
    record("最小种植面积", minimum_area_violations, "亩", "露天地/水浇地至少5亩，大棚至少0.3亩")

    max_plot_count = int(metrics["management_constraints"]["max_plots_per_crop_per_year_season"])
    plot_sets: dict[tuple[int, int, str], set[str]] = defaultdict(set)
    for (year, plot_id, season, crop_id), area in area_by_key.items():
        if area > TOLERANCE:
            plot_sets[(year, crop_id, season)].add(plot_id)
    max_plot_violations = []
    for key, plot_ids in plot_sets.items():
        if len(plot_ids) > max_plot_count:
            max_plot_violations.append((str(key), float(len(plot_ids) - max_plot_count)))
    record("每作物最大地块数", max_plot_violations, "块", "每种作物每年每季最多进入7个地块")
    ...
    for (year, crop_id, season), quantity in production.items():
        ...
        normal_sales = min(quantity, demand.get((crop_id, season), 0.0))
        revenue_by_year[year] += 0.5 * quantity * price + 0.5 * normal_sales * price
    ...
    write_csv(
        RESULTS_DIR / "q1_full_discount_constraint_audit.csv",
