...
CONFIG_PATH = PROJECT_ROOT / "config" / "q1_full_discount.json"
...
def main() -> None:
    ...
    model.setOptionValue("log_file", str(LOGS_DIR / "q1_full_discount_solver.log"))
    ...
    for year in years:
        for plot_id, season, crop_id in eligible_combinations:
            ...
            x_var = model.addVariable(
                lb=0.0,
                ub=plot["area_mu"],
                obj=0.5 * parameter["price"] * parameter["yield"] - parameter["cost"],
            ...
            model.addConstr(
                ...
                name=f"minimum_area_{year}_{plot_id}_{season}_{crop_id}",
            )
    ...
    for year in years:
        for crop_id, season in sorted(crop_season_prices):
            ...
            sales_var = model.addVariable(
                lb=0.0,
                ub=demand,
                obj=0.5 * crop_season_prices[(crop_id, season)],
                type=highspy.HighsVarType.kContinuous,
    ...
    for (year, crop_id, season), flags in y_by_year_crop_season.items():
        model.addConstr(
            variable_sum(flags) <= max_plot_count,
            name=f"max_plot_count_{year}_{crop_id}_{season}",
        )
    ...
    # 用上一轮已通过审计的可行方案热启动，避免重新从低质量保底解开始。
    warm_start_status = None
    warm_start_relative = config.get("warm_start_solution")
    if warm_start_relative:
        warm_start_path = PROJECT_ROOT / warm_start_relative
        if warm_start_path.exists():
            start_rows = read_csv(warm_start_path.name) if warm_start_path.parent == CLEAN_DIR else None
            if start_rows is None:
                with warm_start_path.open("r", encoding="utf-8-sig", newline="") as file:
                    start_rows = list(csv.DictReader(file))
            start_area = {
                (int(row["year"]), row["plot_id"], row["season"], int(row["crop_id"])): float(row["area_mu"])
                for row in start_rows
            }
            start_production: dict[tuple[int, int, str], float] = defaultdict(float)
            indices: list[int] = []
            values: list[float] = []
            for key, variable in x.items():
                area_value = start_area.get(key, 0.0)
                indices.append(variable.index)
                values.append(area_value)
                indices.append(y[key].index)
                values.append(1.0 if area_value > tolerance else 0.0)
                year, plot_id, season, crop_id = key
                parameter = parameter_by_key[(crop_id, plot_by_id[plot_id]["land_type"], season)]
                start_production[(year, crop_id, season)] += area_value * parameter["yield"]
            for key, variable in water_mode.items():
                year, plot_id = key
                indices.append(variable.index)
                values.append(1.0 if start_area.get((year, plot_id, "single", 16), 0.0) > tolerance else 0.0)
            for key, variable in normal_sales.items():
                year, crop_id, season = key
                indices.append(variable.index)
                values.append(min(start_production[key], demand_by_key.get((crop_id, season), 0.0)))
    ...
    for key, variable in normal_sales.items():
        ...
        price = crop_season_prices[(crop_id, season)]
        revenue = 0.5 * production * price + 0.5 * normal_sales_value * price
    ...
    write_csv(
        RESULTS_DIR / "q1_full_discount_solution_long.csv",
    ...
    write_csv(
        RESULTS_DIR / "q1_full_discount_yearly_profit.csv",
    ...
    write_csv(
        RESULTS_DIR / "q1_full_discount_crop_summary.csv",
    ...
    metrics = {
        ...
        "surplus_policy": "discount_50_percent",
        "management_constraints": {
            "minimum_area": True,
            "maximum_plot_count": True,
        ...
        "warm_start": {
            "source": warm_start_relative,
            "set_status": warm_start_status,
        },
    ...
    (RESULTS_DIR / "q1_full_discount_metrics.json").write_text(json.dumps(metrics, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (LOGS_DIR / "q1_full_discount_solver_summary.log").write_text(
        "\n".join(
            [
